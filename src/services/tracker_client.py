...
class TrackerClient:
    async def fetch_status(self, dhl_id: str, token: str) -> dict:
    ...
    async def fetch_shipment(self, dhl_id: str, token: str) -> dict:
        url = f"{settings.SHIPMENT_TRACKER_BASE_URL}/shipments/{dhl_id}"
        return await self._get(url, token)

    async def fetch_current_user(self, token: str) -> dict:
        url = f"{settings.SHIPMENT_TRACKER_BASE_URL}/auth/me"
        return await self._get(url, token)
